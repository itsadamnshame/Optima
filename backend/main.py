    ...
    try:
        ...
        def run_quantitative_branch():
            ...
            def process_item(item):
                ...
                forecast = preprocess_and_forecast_item(item_df, end_date, item, active_id)
            ...
            # Use at most 1 worker on 1-2 CPU machines to avoid thread contention and memory pressure.
            max_workers = 1 if (os.cpu_count() or 1) <= 2 else min((os.cpu_count() or 1) - 1, 2)
        ...
        # Cap bundles to top N from each category
        capped_bundles = rules_df.head(50).to_dict(orient="records")
        
        # Cap chart data: only last 30 days historical + all forecasted future
        if not forecast_df.empty:
            hist_data = forecast_df[forecast_df['type'] == 'historical'].tail(30)
            future_data = forecast_df[forecast_df['type'] == 'future']
            capped_chart = pd.concat([hist_data, future_data], ignore_index=True)
        else:
            capped_chart = forecast_df
        
        res_data = {
            "status": "success",
            "recommendations": final_advice,
            "bundles": capped_bundles,
            "chart_data": json.loads(capped_chart.to_json(orient="records")),
